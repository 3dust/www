  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,#f8fafc_0%,#eef4ff_42%,#ffffff_100%)] text-slate-900">
      <header className="sticky top-0 z-20 border-b border-white/60 bg-white/85 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center gap-8 px-6 py-5 lg:px-10">
          ...
          <nav className="hidden items-center gap-2 md:flex">
            {navItems.map((item) => (
              <button
                key={item}
                type="button"
                className="flex items-center gap-1 rounded-full px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-100 hover:text-slate-950"
              >
                <span>{item}</span>
        ...
        <div className="border-t border-slate-200/80 bg-white/70 md:hidden">
          <div className="mx-auto flex max-w-7xl flex-wrap gap-x-3 gap-y-2 px-6 py-3">
            {navItems.map((item) => (
              <button
                key={item}
                type="button"
